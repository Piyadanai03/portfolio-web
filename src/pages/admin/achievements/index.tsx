...
const AdminAchievements = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("คุณแน่ใจหรือไม่ที่จะลบข้อมูลความสำเร็จนี้?")) return;
    try {
      await deleteAchievement(id);
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : String(err);
      setSaveError(errorMessage || "เกิดข้อผิดพลาด");
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500">
            จัดการประวัติการอบรมและรางวัลที่คุณได้รับ
          </p>
        </div>
        <button
        ...
        {achievements.length === 0 ? (
          ...
          achievements.map((item) => {
            ...
            return (
              ...
              <div
                key={item.id}
                className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-white hover:border-slate-300 transition-all group animate-fade-in"
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 text-2xl shadow-sm border border-slate-100 ${
                      item.category === "award"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-2xl max-h-[95vh] overflow-y-auto animate-fade-in shadow-2xl rounded-2xl">
            {saveError && (
              <div className="sticky top-0 z-10 bg-red-50 border-b border-red-200 p-4 text-red-700 text-sm font-medium">
                ❌ {saveError}
              </div>
            )}
            <AchievementForm
